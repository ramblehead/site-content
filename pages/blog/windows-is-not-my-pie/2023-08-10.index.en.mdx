...
how to setup kernel's `usb_gadget`, including USB device descriptor, from the
shell and how DHCP server can be used at Raspberry Pi side, so the host computer
does not require setting IP address manually. In my tests, Ben's configuration
worked perfectly well on Ubuntu 22.04 LTS. Windows 10 and Windows 11 still
recognise it as a COM port.
...
It looks like they cracked this problem in 2017 or earlier!
...
no additional changes, simultaneously supporting both RNDIS and CDC-ECM USB
